...
TEST(TwoVariableLessEncodingTest, GEOM20_NonOptimal_DummyUpperBound_AllFlagCombos)
{
    for (const bool symm : {false, true})
    {
        for (const bool heur : {false, true})
        {
            ...
            solve_expect(BCPSolver::TwoVariablesLess, "../dataset/GEOM20.col", ub, symm, heur, false, false,
        ...
        for (const bool symm : {false, true})
        {
            for (const bool heur : {false, true})
            {
                SCOPED_TRACE(std::string(path) + " / symmetry=" + (symm ? "on" : "off") + " / heuristic=" +
                    (heur ? "on" : "off"));
                solve_expect(BCPSolver::TwoVariablesLess, path, -1, symm, heur, true, false, SolverStatus::OPTIMAL,
                             expected_span);
            }
        }
    }
    ...
    for (const auto& [path, expected_span] : cases)
    {
        for (const bool symm : {false, true})
        {
            for (const bool heur : {false, true})
            {
                SCOPED_TRACE(std::string(path) + " / symmetry=" + (symm ? "on" : "off") + " / heuristic=" +
                    (heur ? "on" : "off"));
                solve_expect(BCPSolver::TwoVariablesLess, path, -1, symm, heur, true, true, SolverStatus::OPTIMAL,
                             expected_span);
            }
        }
    }
